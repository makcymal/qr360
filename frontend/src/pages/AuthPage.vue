    ...
    <div class="bg whit-bg">
        ...
        <div class="auth-form">
            ...
            <input
                ...
                :disabled="true"
                type="text"
                class="input sml invisible"
            />

            <button
                v-if="confIsValid"
                @click="login"
                class="login-btn orng-bg active"
            >
...
export default {
    ...
    methods: {
        onInput(event) {
            ...
            for (
                var i = 3;
                i < event.target.value.length && phone_len < 16;
                ++i
            ) {
        ...
        authChoice(mode) {
            ...
            if (!this.phoneIsValid) {
                ...
                if (this.userExists)
                    this.authMsg =
                        "Введите код, полученный от нашего бота @aboba в личном сообщении";
                else
                    this.authMsg =
                        "Напишите /start нашему боту @aboba. <br> Он вышлет в личном сообщении код регистрации";
            } else if (mode == "p") {
                if (this.userExists) this.authMsg = "Введите пароль:";
                else
                    this.authMsg =
                        "Придумайте пароль. <br> Не меньше 8 символов, хотя бы одна цифра, одна буква и один символ из . , - + /";
            }
        ...
        onConf() {
            ...
            if (this.authMode == "t") {
                ...
                if (digit && alpha && sym) this.confIsValid = true;
                else this.confIsValid = false;
            }
        },
        ...
        login() {
            if (this.authMode == "p") {
                let username = "";
                for (var i = 3; i < this.phone.length && i < 16; ++i)
                    if (this.phone[i] != " ") username += this.phone[i];
